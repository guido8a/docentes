                ...
                <table class="table table-condensed table-bordered table-striped">
                    ...
                    <g:each in="${materias}" var="materia">
                        <tr data-id="${materia.id}">
                            <td style="width: 8%">${materia?.materiaDictada?.materia?.codigo}</td>
                            <td style="width: 34%">${materia?.materiaDictada?.materia?.nombre}</td>
                            <td style="width: 15%">${materia?.materiaDictada?.curso?.nombre}</td>
                            <td style="width: 8%">${materia?.materiaDictada?.paralelo}</td>
                            <td style="width: 25%">${materia?.materiaDictada?.profesor?.nombre + ' ' + materia?.materiaDictada?.profesor?.apellido}</td>
                            <td style="width: 25%">${materia?.materiaDictada?.profesor?.escuela?.nombre}</td>
...
<script type="text/javascript">
    $(".btnBorrar").click(function () {
        ...
        bootbox.confirm("<i class='fa fa-exclamation-triangle fa-3x text-danger text-shadow'>" +
                "</i> Está seguro de desasignar esta materia del alumno?", function (result){
                if(result){
                    $.ajax({
                        type: 'POST',
